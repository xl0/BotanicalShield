...
live_botanical_packet_handler()
{
   ...
   int ax, ay, az, gx, gy, gz;
    ...
    accelgyro.add_values(float(ax),float(ay),float(ax),float(ax),float(ay),float(ay));
   // write_accelgyro(ax,ay,az,gx,gy,gz);
}
int
...
info_packet_handler()
{
  ...
  //added
  buf_i=9;
  //startYear = get_int_from_buffer();
  startHour = get_int_from_buffer();
  //println("Year : "+startYear);
  println("Hour : "+startHour);
  buf_i++;
  startMinute = get_int_from_buffer();
  println("Minute : "+startMinute);
  buf_i++;
  startSecond = get_int_from_buffer();
  println("Second : "+startSecond);
  buf_i++;
  startDay = get_int_from_buffer();
  println("Second : "+startDay);
  buf_i++;
  startDayOfWeek = get_int_from_buffer();
  println("Day of week : "+startDayOfWeek);
  buf_i++;
  startMonth = get_int_from_buffer();
  println("Month : "+startMonth);
  buf_i++;
  startYear = get_int_from_buffer()+1900;
  println("Year : "+startYear);
